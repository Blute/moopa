<cfcomponent>
    ...
    <cffunction name="mergeVideoMetadata" access="private" returntype="void" output="false" hint="Atomically merge keys into moo_video.metadata so concurrent metadata writers are never clobbered">
    ...
    <cffunction name="buildVideoResponse" access="private" returntype="struct" output="false">
        ...
        <cfset var stored_thumbnail = trim(arguments.video_record.thumbnail ?: "") />
        ...
        <cfreturn {
            ...
            video_thumbnail: (len(stream_thumbnail) AND stream_ready ? stream_thumbnail : stored_thumbnail)
        } />
    </cffunction>
    ...
    <cffunction name="pollVideoStatuses" access="public" returntype="array" output="false">
        ...
        <cfloop array="#clean_ids#" index="pending_id">
            ...
            <!--- Merge only the keys this poll changed so concurrent metadata writers
                  (e.g. project background threads) are never clobbered by a whole-metadata save --->
