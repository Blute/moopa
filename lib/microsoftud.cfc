<cfcomponent displayname="Microsoft User Directory">
	...
	<cffunction name="authenticate" access="public" output="false" returntype="struct" hint="Attempts to process a user. Runs every time the login form is loaded.">
		...
		<cfelseif isDefined("url.type") and url.type eq "mudLogin" and structkeyexists(url,"code")>
			...
			<!--- <cftry> --->
				...
				<cfset stTokens = getTokens(url.code, getRedirectURL()) />
				<cfset var stProfile = getMicrosoftProfile(stTokens.access_token) />
	...
	<cffunction name="isEnabled" access="public" output="false" returntype="boolean" hint="Returns true if this user directory is active. This function can be overridden to check for the existence of config settings.">
		...
		<cfreturn len(variables.CLIENTID) and len(variables.CLIENTSECRET) />
	</cffunction>
	...
	<cffunction name="parseProxy" access="private" output="false" returntype="struct">
		...
		<cfif len(variables.PROXY)>
			<cfif listlen(variables.PROXY,"@") eq 2>
				<cfset stResult.login = listfirst(variables.PROXY,"@") />
			...
			<cfset stResult.server = listlast(variables.PROXY,"@") />
	...
	<cffunction name="getAuthorisationURL" access="public" output="false" returntype="string">
		...
		<cfreturn "https://login.microsoftonline.com/#variables.TENANT#/oauth2/v2.0/authorize?response_type=code&client_id=#variables.CLIENTID#&redirect_uri=#urlencodedformat(arguments.redirectURL)#&scope=#urlencodedformat(variables.SCOPE)#&access_type=offline&prompt=#urlencodedformat(arguments.prompt)#&state=#urlencodedformat(arguments.state)#" />
	</cffunction>

	<cffunction name="getTokens"  returntype="struct">
		...
		<cfset stAttr.url = "https://login.microsoftonline.com/#variables.tenant#/oauth2/v2.0/token" />
		<cfset stAttr.method = "POST" />

		<cfif len(variables.PROXY)>
			<cfset structappend(stAttr,parseProxy(variables.PROXY)) />
		</cfif>
		...
		<cfhttp attributeCollection="#stAttr#" result="stResponse">
			...
			<cfhttpparam type="formfield" name="scope" value="#variables.scope#" />
		</cfhttp>

		<cfif not stResponse.statuscode eq "200 OK">
            <cfdump var="#stResponse#" /><cfabort>
            <cfthrow message="Error accessing Microsoft Auth API: #stResponse.statuscode#" />
			<!--- <cfset throwError(message="Error accessing Microsoft Auth API: #stResponse.statuscode#",endpoint="https://login.microsoftonline.com/#variables.tenant#/oauth2/v2.0/token",response=trim(stResponse.filecontent),args=arguments,stAttr=stAttr) /> --->
		</cfif>
	...
	<cffunction name="getAccessToken" access="public" output="false" returntype="string">
		...
		<cfset var stResponse = "">
		<cfset var stResult = structnew() />
		<cfset var stProxy = parseProxy(variables.PROXY) />

		<cfif len(arguments.refresh_token?:'') and datecompare(arguments.access_token_expires,now()) lt 0>
			<cfhttp url="https://login.microsoftonline.com/#variables.TENANT#/oauth2/v2.0/token" method="POST" attributeCollection="#stProxy#" result="stResponse">
			...
			<cfif not stResponse.statuscode eq "200 OK">
                <cfthrow message="Error accessing Microsoft Auth API: #stResponse.statuscode#" />
				<!--- <cfset throwError(message="Error accessing Microsoft Auth API: #stResponse.statuscode#",endpoint="https://login.microsoftonline.com/#variables.tenant#/oauth2/v2.0/token",response=stResponse.filecontent,argumentCollection=arguments) /> --->
			</cfif>
		...
		<cfelseif not isdefined("arguments.refresh_token")>
            <cfthrow message="Error accessing Microsoft Auth API: access token has expired and no refresh token is available" />
	...
	<cffunction name="getMicrosoftProfile" output="false" returntype="struct">
		...
		<cfif not stResponse.statuscode eq "200 OK">
            <cfthrow message="Error accessing Microsoft Graph API: #stResponse.statuscode#" />
	...
	<cffunction name="getRedirectURL" access="public" output="false" returntype="string" hint="For use with getAuthorisationURL and getRefreshToken">
		<cfreturn "#server.system.environment.BASE_URL#" />
	</cffunction>
